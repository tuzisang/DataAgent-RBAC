...
- **WHEN** 管理员调用 `PUT /api/admin/users/{id}/agent-visibility` 传入 `{ "agentIds": [1, 2, 3] }`
- **THEN** `sys_user_agent_visibility` 表中该用户的记录被替换为这三条，旧记录被删除

#### Scenario: Clear all agent visibility for a user
- **WHEN** 管理员调用 `PUT /api/admin/users/{id}/agent-visibility` 传入 `{ "agentIds": [] }`
...
### Requirement: User management page has agent assignment button
