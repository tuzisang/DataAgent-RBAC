...
| **RBAC 权限认证** | 基于 Sa-Token 的 4 角色 RBAC 体系（超级管理员/管理员/分析师/访客），27 项细粒度权限控制，支持 Agent 可见性分配。 |

## 🔐 RBAC 权限体系

本项目集成 **Sa-Token RBAC** 权限认证系统，提供 4 种预设角色和 27 项细粒度权限：

| 角色 | 权限数 | 说明 |
| :--- | :--- | :--- |
| **super-admin** (超级管理员) | 27 | 拥有全部权限，可管理用户、角色、权限分配 |
| **admin** (管理员) | 24 | 可管理 Agent、数据源、知识库，可发布 Agent 和分配 Agent 可见性 |
| **analyst** (分析师) | 15 | 可创建和编辑自己的 Agent，使用对话功能 |
| **viewer** (访客) | 8 | 仅可查看已发布的 Agent 和使用对话，不可修改任何配置 |

**Agent 可见性控制**: 非管理员用户可查看已发布的 Agent、自己创建的 Agent、以及管理员分配的 Agent。管理员可通过用户管理页面为用户分配特定 Agent 的可见权限。
